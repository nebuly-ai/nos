...
import (
	ts_partitioning "github.com/nebuly-ai/nebulnetes/internal/partitioning/ts"
...
func TestSliceCalculator(t *testing.T) {
		...
		},
		{
			...
			pod: factory.BuildPod("ns-1", "pd-1").WithContainer(
				factory.BuildContainer("c-1", "im-1").
					...
					WithScalarResourceRequest(timeslicing.ProfileName("nvidia.com/gpu-10gb").AsResourceName(), 1).
					Get(),
			).Get(),
			expected: map[gpu.Slice]int{
				timeslicing.ProfileName("nvidia.com/gpu-10gb"): 1,
			},
		},
	}
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			slices := ts_partitioning.NewSliceCalculator().GetRequestedSlices(tt.pod)
